...
public class SakController {
    ...
    private static final org.slf4j.Logger log = org.slf4j.LoggerFactory.getLogger(SakController.class);
    ...
    public List<BucinfoDto> hentTilknyttedeBucer(@PathVariable Long gsakSaksnummer, @RequestParam(required = false) List<String> statuser) {
        return saksrelasjonService.finnVedGsakSaksnummer(gsakSaksnummer).stream().map(FagsakRinasakKobling::getRinaSaksnummer).map(euxService::finnBUC).flatMap(Optional::stream).map(buc -> BucinfoDto.av(buc, statuser, euxService.hentRinaUrl(buc.getId()))).collect(Collectors.toList());
    }

    @PostMapping
    @ApiResponse(description = "Lagrer en saksrelasjon mellom en rinasak og en gsak-sak")
    public ResponseEntity<Void> lagreSaksrelasjon(@RequestBody SaksrelasjonDto saksrelasjonDto) throws ValidationException {
        validerSaksrelasjonDto(saksrelasjonDto);
        saksrelasjonService.lagreKobling(saksrelasjonDto.getGsakSaksnummer(), saksrelasjonDto.getRinaSaksnummer(), BucType.valueOf(saksrelasjonDto.getBucType()));
        return ResponseEntity.ok().build();
    }

    @GetMapping
    @ApiResponse(description = "Søker etter saksrelasjon basert på enten rinaSaksnummer eller gsakSaksnummer")
    public List<SaksrelasjonDto> hentSaksrelasjon(@PathParam("rinaSaksnummer") String rinaSaksnummer, @PathParam("gsakSaksnummer") Long gsakSaksnummer) throws ValidationException {
        if (!StringUtils.hasText(rinaSaksnummer) && gsakSaksnummer != null) {
            return saksrelasjonService.finnVedGsakSaksnummer(gsakSaksnummer).stream().map(SaksrelasjonDto::av).collect(Collectors.toList());
        } else if (StringUtils.hasText(rinaSaksnummer) && gsakSaksnummer == null) {
            return saksrelasjonService.søkEtterSaksnummerFraRinaSaksnummer(rinaSaksnummer).map(saksnummer -> Collections.singletonList(new SaksrelasjonDto(saksnummer, rinaSaksnummer, null))).orElse(Collections.emptyList());
        }
