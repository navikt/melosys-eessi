...
public final class OpprettJournalpostRequestMapper {
    ...
    private static final org.slf4j.Logger log = org.slf4j.LoggerFactory.getLogger(OpprettJournalpostRequestMapper.class);
    public static final double MIN_INFLATE_RATIO = 0.001;
    public static final float PDF_MARGIN = 20.0F;

    public static OpprettJournalpostRequest opprettInngaaendeJournalpost(final SedHendelse sedHendelse, final SedMedVedlegg sedMedVedlegg, final Sak sak, final String dokumentTittel, final String behandlingstema, final String personIdent, final SedMetrikker sedMetrikker) {
        return opprettJournalpostRequest(JournalpostType.INNGAAENDE, sedHendelse, sedMedVedlegg, sak, dokumentTittel, behandlingstema, personIdent, sedMetrikker);
    }

    public static OpprettJournalpostRequest opprettUtgaaendeJournalpost(final SedHendelse sedHendelse, final SedMedVedlegg sedMedVedlegg, final Sak sak, final String dokumentTittel, final String behandlingstema, final String personIdent, final SedMetrikker sedMetrikker) {
        return opprettJournalpostRequest(JournalpostType.UTGAAENDE, sedHendelse, sedMedVedlegg, sak, dokumentTittel, behandlingstema, personIdent, sedMetrikker);
    }

    private static OpprettJournalpostRequest opprettJournalpostRequest(final JournalpostType journalpostType, final SedHendelse sedHendelse, final SedMedVedlegg sedMedVedlegg, final Sak sak, final String dokumentTittel, final String behandlingstema, final String personIdent, final SedMetrikker sedMetrikker) {
        return OpprettJournalpostRequest.builder().avsenderMottaker(getAvsenderMottaker(journalpostType, sedHendelse)).behandlingstema(behandlingstema).bruker(isNotEmpty(personIdent) ? lagBruker(personIdent) : null).dokumenter(dokumenter(sedHendelse, sedMedVedlegg, dokumentTittel, sedMetrikker)).eksternReferanseId(sedHendelse.getSedId()).journalfoerendeEnhet("4530").journalpostType(journalpostType).kanal("EESSI").sak(sak != null ? OpprettJournalpostRequest.Sak.builder().arkivsaksnummer(sak.getId()).build() : null).tema(sak != null ? sak.getTema() : temaForSedTypeOgJournalpostType(sedHendelse.getSedType(), journalpostType)).tittel(dokumentTittel).tilleggsopplysninger(Arrays.asList(Tilleggsopplysning.builder().nokkel("rinaSakId").verdi(sedHendelse.getRinaSakId()).build(), Tilleggsopplysning.builder().nokkel("rinaDokumentId").verdi(sedHendelse.getRinaDokumentId()).build())).build();
    }

    private static Bruker lagBruker(final String personIdent) {
        return Bruker.builder().id(personIdent).idType(BrukerIdType.FNR).build();
    }

    private static AvsenderMottaker getAvsenderMottaker(final JournalpostType type, final SedHendelse sedHendelse) {
        return AvsenderMottaker.builder().id(type == JournalpostType.UTGAAENDE ? sedHendelse.getMottakerId() : sedHendelse.getAvsenderId()).navn(type == JournalpostType.UTGAAENDE ? sedHendelse.getMottakerNavn() : sedHendelse.getAvsenderNavn()).idType(AvsenderMottaker.IdType.UTL_ORG).build();
    }

    private static List<Dokument> dokumenter(final SedHendelse sedHendelse, final SedMedVedlegg sedMedVedlegg, final String dokumentTittel, final SedMetrikker sedMetrikker) {
        final List<Dokument> dokumenter = new ArrayList<>();
        dokumenter.add(dokument(sedHendelse.getSedType(), dokumentTittel, JournalpostFiltype.PDFA, sedMedVedlegg.getSed().getInnhold()));
        dokumenter.addAll(vedlegg(sedHendelse, sedMedVedlegg.getVedlegg(), sedMetrikker));
        return dokumenter;
    }

    private static Dokument dokument(final String sedType, final String filnavn, final JournalpostFiltype journalpostFiltype, final byte[] innhold) {
        return Dokument.builder().dokumentvarianter(Collections.singletonList(DokumentVariant.builder().filtype(journalpostFiltype).fysiskDokument(innhold).variantformat("ARKIV").build())).sedType(sedType).tittel(filnavn).build();
    }
    ...
    private static List<Dokument> vedlegg(final SedHendelse sedHendelse, final List<SedMedVedlegg.BinaerFil> vedleggListe, SedMetrikker sedMetrikker) {
        ...
        for (SedMedVedlegg.BinaerFil binaerFil : vedleggListe) {
            try {
                ...
                log.error("Kunne ikke konvertere vedlegg %s med MIME-type %s til PDF. RINA saksnummer: %s".formatted(binaerFil.getFilnavn(), binaerFil.getMimeType(), sedHendelse.getRinaSakId()));
    ...
    private static String temaForSedTypeOgJournalpostType(final String sedType, final JournalpostType journalpostType) {
