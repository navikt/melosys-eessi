...
public class OpprettUtgaaendeJournalpostService {
    ...
    private static final org.slf4j.Logger log = org.slf4j.LoggerFactory.getLogger(OpprettUtgaaendeJournalpostService.class);
    ...
    private boolean sedInneholderPersonId(SedHendelse sedHendelse) {
        ...
        var sed = euxService.hentSedMedRetry(rinaSaksId, sedHendelse.getRinaDokumentId());
    ...
    private OpprettJournalpostResponse opprettUtgåendeJournalpost(SedHendelse sedSendt, Sak sak, String navIdent) {
        return journalpostService.opprettUtgaaendeJournalpost(sedSendt, sak, euxService.hentSedMedVedlegg(sedSendt.getRinaSakId(), sedSendt.getRinaDokumentId()), navIdent);
    }

    private String opprettUtgåendeJournalføringsoppgave(SedHendelse sedSendt, String journalpostId, String navIdent) {
        return oppgaveService.opprettUtgåendeJfrOppgave(journalpostId, sedSendt, isNotEmpty(navIdent) ? personFasade.hentAktoerId(navIdent) : null, euxService.hentRinaUrl(sedSendt.getRinaSakId()));
    }

    @java.lang.SuppressWarnings("all")
    public OpprettUtgaaendeJournalpostService(final SaksrelasjonService saksrelasjonService, final JournalpostService journalpostService, final EuxService euxService, final PersonFasade personFasade, final OppgaveService oppgaveService, final SedMetrikker sedMetrikker, final PersonIdentifisering personIdentifisering, final SedSendtHendelseRepository sedSendtHendelseRepository, final Unleash unleash) {
        ...
        this.sedSendtHendelseRepository = sedSendtHendelseRepository;
