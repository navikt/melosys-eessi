...
public class JournalpostSedKoblingService {
    ...
    private static final org.slf4j.Logger log = org.slf4j.LoggerFactory.getLogger(JournalpostSedKoblingService.class);
    ...
    public JournalpostSedKoblingService(JournalpostSedKoblingRepository journalpostSedKoblingRepository, EuxService euxService, SaksrelasjonService saksrelasjonService, SafConsumer safConsumer, MelosysEessiMeldingMapperFactory melosysEessiMeldingMapperFactory) {
    ...
    public boolean erASedAlleredeBehandlet(String rinaSaksnummer) {
        return journalpostSedKoblingRepository.findByRinaSaksnummer(rinaSaksnummer).stream().anyMatch(JournalpostSedKobling::erASed);
    }
    ...
    private MelosysEessiMelding opprettEessiMelding(JournalpostSedKobling journalpostSedKobling) {
        ...
        Long gsakSaksnummer = saksrelasjonService.finnVedRinaSaksnummer(journalpostSedKobling.getRinaSaksnummer()).map(FagsakRinasakKobling::getGsakSaksnummer).orElse(null);
        return opprettMelosysEessiMelding(sed, journalpostSedKobling.getSedId(), journalpostSedKobling.getRinaSaksnummer(), journalpostSedKobling.getSedType(), journalpostSedKobling.getBucType(), organisation.getId(), organisation.getCountryCode(), journalpostSedKobling.getJournalpostID(), gsakSaksnummer != null ? gsakSaksnummer.toString() : null, Integer.parseInt(journalpostSedKobling.getSedVersjon()) != 1, journalpostSedKobling.getSedVersjon());
    }
    ...
    private Optional<MelosysEessiMelding> opprettEessiMelding(String rinaSaksnummer, String journalpostID) {
        ...
        return Optional.of(opprettMelosysEessiMelding(sed, sedID, rinaSaksnummer, sedType, buc.getBucType(), organisation.getId(), organisation.getCountryCode(), journalpostID, null, false, sedVersjon));
    }

    public JournalpostSedKobling lagre(String journalpostID, String rinaSaksnummer, String sedID, String sedVersjon, String bucType, String sedType) {
        return journalpostSedKoblingRepository.save(new JournalpostSedKobling(journalpostID, rinaSaksnummer, sedID, sedVersjon, bucType, sedType));
    }

    private MelosysEessiMelding opprettMelosysEessiMelding(SED sed, String sedId, String rinaSaksnummer, String sedType, String bucType, String avsenderID, String landkode, String journalpostID, String saksnummer, boolean erEndring, String sedVersjon) {
        return melosysEessiMeldingMapperFactory.getMapper(SedType.valueOf(sedType)).map(null, sed, sedId, rinaSaksnummer, sedType, bucType, avsenderID, landkode, journalpostID, null, saksnummer, erEndring, sedVersjon);
    }
}
