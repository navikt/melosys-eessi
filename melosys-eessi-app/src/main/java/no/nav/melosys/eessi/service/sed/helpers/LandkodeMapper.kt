...
public final class LandkodeMapper {
    ...
    private static final org.slf4j.Logger log = org.slf4j.LoggerFactory.getLogger(LandkodeMapper.class);
    ...
    public static String finnLandkodeIso3ForIdentRekvisisjon(String landkodeIso2, boolean skalReturnereNullForUkjent) {
        ...
        return ISO3_TIL_ISO2_LANDKODER_MAP.entrySet().stream().filter(entry -> entry.getValue().equals(mapTilNavLandkode(landkodeIso2))).map(Map.Entry::getKey).findFirst().orElse(skalReturnereNullForUkjent ? null : UKJENT_LANDKODE_ISO3);
    }
