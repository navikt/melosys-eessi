...
public class SedMottakService {
    ...
    private static final org.slf4j.Logger log = org.slf4j.LoggerFactory.getLogger(SedMottakService.class);
    ...
    public void behandleSedMottakHendelse(SedMottattHendelse sedMottattHendelse) {
        ...
        final var sed = euxService.hentSedMedRetry(sedMottattHendelse.getSedHendelse().getRinaSakId(), sedMottattHendelse.getSedHendelse().getRinaDokumentId());
        log.info("Søker etter person for SED");
        personIdentifisering.identifiserPerson(lagretHendelse.getSedHendelse().getRinaSakId(), sed).ifPresentOrElse(ident -> bucIdentifisertService.lagreIdentifisertPerson(lagretHendelse.getSedHendelse().getRinaSakId(), ident), () -> opprettOppgaveIdentifisering(lagretHendelse, sed));
    ...
    private boolean erXSedBehandletUtenASed(SedHendelse sedHendelse) {
        ...
        if (sedHendelse.getSedType().equals(SedType.X007.name())) {
            ...
            boolean sedTypeErX007OgNorgeErSakseier = buc.getParticipants().stream().anyMatch(p -> p.getRole().equals(Participant.ParticipantRole.SAKSEIER) && p.getOrganisation().getId().equals(rinaInstitusjonsId));
    ...
    private void opprettOppgaveIdentifisering(SedMottattHendelse sedMottatt, SED sed) {
        ...
        bucIdentifiseringOppgRepository.findByRinaSaksnummer(rinaSaksnummer).stream().filter(this::oppgaveErÅpen).findFirst().ifPresentOrElse(bucIdentifiseringOppg -> log.info("Identifiseringsoppgave {} finnes allerede for rinasak {}", bucIdentifiseringOppg.getOppgaveId(), rinaSaksnummer), () -> opprettOgLagreIdentifiseringsoppgave(sedMottatt, sed));
    }
    ...
    private void opprettOgLagreIdentifiseringsoppgave(SedMottattHendelse sedMottattHendelse, SED sed) {
        ...
        bucIdentifiseringOppgRepository.save(BucIdentifiseringOppg.builder().rinaSaksnummer(sedMottattHendelse.getSedHendelse().getRinaSakId()).oppgaveId(oppgaveID).versjon(1).build());
    ...
    private String opprettOgLagreIndentifiseringsoppgave(SedMottattHendelse sedMottattHendelse, SED sed, String journalpostID) {
        ...
        if (personFraSed != null && !harNorskPersonnummer(personFraSed)) {
            ...
            return oppgaveService.opprettOppgaveTilIdOgFordeling(journalpostID, sedMottattHendelse.getSedHendelse().getSedType(), sedMottattHendelse.getSedHendelse().getRinaSakId(), lenkeForRekvirering);
        }
        return oppgaveService.opprettOppgaveTilIdOgFordeling(journalpostID, sedMottattHendelse.getSedHendelse().getSedType(), sedMottattHendelse.getSedHendelse().getRinaSakId());
    }

    private boolean harNorskPersonnummer(Person personFraSed) {
        return personFraSed.finnNorskPin().map(Pin::getIdentifikator).flatMap(FnrUtils::filtrerUtGyldigNorskIdent).isPresent();
    }
    ...
    private String opprettJournalpost(SedMottattHendelse sedMottattHendelse, String navIdent) {
        ...
        String journalpostID = opprettInngaaendeJournalpostService.arkiverInngaaendeSedUtenBruker(sedMottattHendelse.getSedHendelse(), sedMedVedlegg, navIdent);
    ...
    @java.lang.SuppressWarnings("all")
    public SedMottakService(final EuxService euxService, final PersonFasade personFasade, final OpprettInngaaendeJournalpostService opprettInngaaendeJournalpostService, final OppgaveService oppgaveService, final SedMottattHendelseRepository sedMottattHendelseRepository, final BucIdentifiseringOppgRepository bucIdentifiseringOppgRepository, final JournalpostSedKoblingService journalpostSedKoblingService, final SedMetrikker sedMetrikker, final PersonIdentifisering personIdentifisering, final BucIdentifisertService bucIdentifisertService) {
