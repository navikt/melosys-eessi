...
@Service
@Primary
class EuxService @Autowired constructor(
    ...
    private fun filtrerPåLandkoder(institusjon: Institusjon, landkoder: Collection<String>): Boolean {
        return landkoder.isEmpty() || landkoder.any { it.equals(institusjon.landkode, ignoreCase = true) }
    }
    ...
    fun opprettOgSendSed(sed: SED?, rinaSaksnummer: String?) {
        ...
        log.info("SED {} sendt i sak {}", sed?.sedType, rinaSaksnummer)
    }

    fun validerBucHandling(rinaSaksnummer: String?, aksjon: Aksjoner) {
    ...
    fun validerSedHandling(rinaSaksnummer: String?, sedId: String?, aksjon: Aksjoner) {
    ...
    fun hentSed(rinaSaksnummer: String?, dokumentId: String?): SED? {
    ...
    fun hentBucer(bucSearch: BucSearch): List<BucInfo> {
        return euxConsumer.finnRinaSaker(bucSearch.bucType, bucSearch.status)
    }

    fun hentBuc(rinaSaksnummer: String): BUC {
        return euxConsumer.hentBUC(rinaSaksnummer)
    }

    fun finnBUC(rinaSaksnummer: String?): Optional<BUC> {
        return try {
    ...
    fun bucHandlingErMulig(rinaSaksnummer: String?, aksjon: Aksjoner): Boolean {
        return euxConsumer.hentBucHandlinger(rinaSaksnummer).any { it.split(" ")[2] == aksjon.hentHandling() }
    }

    fun sedHandlingErMulig(rinaSaksnummer: String?, dokumentId: String?, handling: Aksjoner): Boolean {
        return euxConsumer.hentSedHandlinger(rinaSaksnummer, dokumentId).any { it == handling.hentHandling() }
    }
    ...
    fun genererPdfFraSed(sed: SED): ByteArray? {
        return euxConsumer.genererPdfFraSed(sed)
    }

    fun hentRinaUrl(rinaCaseId: String?): String? {
        if (!StringUtils.hasText(rinaCaseId)) {
            throw IllegalArgumentException("Trenger rina-saksnummer for å opprette url til rina")
        }
    ...
    fun settSedJournalstatus(rinaSaksnummer: String, dokumentId: String, versjon: Int, sedJournalstatus: SedJournalstatus) {
        if (!StringUtils.hasText(rinaSaksnummer)) {
            throw IllegalArgumentException("Trenger rina-saksnummer for å oppdatere sed")
        }
        euxRinasakerConsumer.settSedJournalstatus(EuxMelosysSedOppdateringDto(rinaSaksnummer, dokumentId, versjon, sedJournalstatus))
    }

    fun settSakSensitiv(rinaSaksnummer: String) {
        euxConsumer.setSakSensitiv(rinaSaksnummer)
    }

    companion object {
        private val log = LoggerFactory.getLogger(EuxService::class.java)
