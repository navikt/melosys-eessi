...
public class EuxService {
    ...
    private static final org.slf4j.Logger log = org.slf4j.LoggerFactory.getLogger(EuxService.class);
    ...
    @Autowired
    public EuxService(EuxConsumer euxConsumer, BucMetrikker bucMetrikker, EuxRinasakerConsumer euxRinasakerConsumer) {
    ...
    public OpprettBucOgSedResponse opprettBucOgSed(BucType bucType, Collection<String> mottakere, SED sed, Collection<SedVedlegg> vedlegg) {
    ...
    public void validerBucHandling(String rinaSaksnummer, Aksjoner aksjon) {
        if (!bucHandlingErMulig(rinaSaksnummer, aksjon)) {
            throw new ValidationException(String.format("Kan ikke gjøre handling %s på BUC %s" + ", ugyldig handling i Rina", aksjon.hentHandling(), rinaSaksnummer));
        }
    }
    ...
    public void validerSedHandling(String rinaSaksnummer, String sedId, Aksjoner aksjon) {
        if (!sedHandlingErMulig(rinaSaksnummer, sedId, aksjon)) {
            throw new ValidationException(String.format("Kan ikke sende SED på BUC %s, ugyldig handling %s i Rina", rinaSaksnummer, aksjon.hentHandling()));
        }
    }

    public boolean sedErEndring(String sedId, String rinaSaksnummer) {
        var buc = euxConsumer.hentBUC(rinaSaksnummer);
        return buc.getDocuments().stream().filter(document -> document.getId().equals(sedId)).findFirst().filter(document -> document.getConversations().size() > 1).isPresent();
    }
    ...
    public boolean bucHandlingErMulig(String rinaSaksnummer, Aksjoner aksjon) {
        return euxConsumer.hentBucHandlinger(rinaSaksnummer).stream().anyMatch(s -> s.split(" ")[2].equals(aksjon.hentHandling()));
    }

    public boolean sedHandlingErMulig(String rinaSaksnummer, String dokumentId, Aksjoner handling) {
        return euxConsumer.hentSedHandlinger(rinaSaksnummer, dokumentId).stream().anyMatch(s -> s.equals(handling.hentHandling()));
    }
